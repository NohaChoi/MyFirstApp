...
class MainActivity : AppCompatActivity() {
    ...
    private fun fetchNextPair() {
        ...
        var candidates = pool.filter { it.fileName != imageA.fileName && it.playerStats.rating >= lowBound && it.playerStats.rating <= highBound }
    ...
    private fun rateImage(choice: Int) {
        ...
        val updatedWinner = winner.copy(playerStats = winner.playerStats.copy(rating = winnerResult.rating, rd = winnerResult.rd, vol = winnerResult.vol, wins = winner.playerStats.wins + 1))
        imageList[imageList.indexOfFirst { it.fileName == winner.fileName }] = updatedWinner
        val updatedLoser = loser.copy(playerStats = loser.playerStats.copy(rating = loserResult.rating, rd = loserResult.rd, vol = loserResult.vol, losses = loser.playerStats.losses + 1))
